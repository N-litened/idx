...
- `src/com/wotbrew/idx.cljc` — the entire public API (`auto`, `index`, `delete-index`, `unwrap`, `lookup`, `identify`, `pk`, `replace-by`, `ascending`, `descending`, and the property combinators `path`, `match`, `pred`, `pcomp`, `as-key`, `tuple`). Everything under `impl/` is explicitly an implementation detail.
...
- **A property must be a value, not a fresh function.** idx addresses an index by the property's *equality*, and functions compare by identity, so every function-constructing combinator (`juxt`, `partial`, `comp`, a bare `fn`) yields a property that cannot find an index built by another call. Every combinator the library ships is therefore a `defrecord`: `path`, `match`, `pcomp`, `as-key`, and `tuple` — which exists specifically so an *ordered composite* index has a value-equal property, since `match` extracts a map and a map is not `Comparable`. Prefer adding a record over telling callers to hold onto an instance.
...
- **A manually indexed collection refuses a query no index can answer quickly** — `unindexed!` in `idx.cljc` throws when `-manual?` is true and no usable index exists. Only two collections still scan: `auto` (which realises the index instead) and plain unwrapped ones (which never declared any, and whose scan is the documented "query functions also work on normal collections" behaviour). The fallbacks still exist and are still reached by those two, so do not delete them. Before this, a lost index was invisible: correct answers at scan cost, and `ascending` rebuilt a whole sorted map per call. `-manual?` is on the `Idx` protocol and must be implemented in all six `{vector,map,set}.{clj,cljs}` wrappers and both `ext` files.
